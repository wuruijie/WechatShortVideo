...
# Name: %service-name%
# Description: This action sends the dropped items to BetterZip to compress them with the preset %preset-name%. When clicked, BetterZip is activated.
# Handles: Files
...
def dragged():
	...
	pid = BetterZip.queuedArchive_withPreset_withOptions_showUi_(items, preset, {}, False)
	if pid > 0:
		...
		# dz.add_dropbar([url.path() for url in archives])
	else:
